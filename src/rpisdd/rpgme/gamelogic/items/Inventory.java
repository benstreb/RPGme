...
public class Inventory {
	...
	public void loadItemsFromDatabase(Activity activity){
		...
		String[] projection = {
		    ItemFeedEntry.COLUMN_NAME_ITEM_NAME,
		    };
		Log.i("Debug:", "3");
		Cursor cursor = db.query(
		    ItemFeedEntry.TABLE_NAME,  // The table to query
	...
	public void saveItemsToDatabase(Activity activity){
		...
		for(int i=0;i<items.size();i++){
			...
			values.put(ItemFeedEntry.COLUMN_NAME_ITEM_NAME, items.get(i).getName());
			
			//Log.i("Debug:", "Saved quest '" + items.get(i).getName() + "' in database:");
